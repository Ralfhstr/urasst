...
import { addFCategory, getFCategory } from "../actions/AuthAction";
...
const FCategory = () => {
    ...
    const onAddFCategory = async () => {
        if (cname) {
            ...
            console.log(data);
            try {
                const user = await addFCategory(data);
                toggleSuccessModal();
                setTimeout(() => {
                    navigation.replace("FCategory");
                }, 2000);
                navigation.replace("FCategory");
            } catch (error) {
        ...
        <SafeAreaView>
            <HStack p={4} justifyContent={'space-around'} alignItems="center">
                ...
                <Box alignItems="center">
                    <Heading color={'primary.700'} >Finance Category</Heading>
                </Box>
            ...
            <FlatList
                ...
                keyExtractor={(item) => item.fCategoryId}
            />
            <Modal isOpen={isCreateCategoryModalOpen} onClose={() => setCreateCategoryModalOpen(false)} size="lg">
                <Modal.Content>
                    ...
                    <Modal.Footer>
                        <Button flex="1" onPress={() => { onAddFCategory(); }}>
                            Submit
                        </Button>
                    </Modal.Footer>
                </Modal.Content>
