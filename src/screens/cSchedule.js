...
const CSchedule = () => {
  ...
  const onAddSchedule = async () => {
    console.log("Nilai startTime sebelum pemanggilan onAddSchedule:", startTime);

    if (activity && sdescription && selectedBox && selectedDate && startTime && endTime) {
      const data = {
        activity: activity,
      ...
      console.log("Data setelah penambahan jadwal:", data);

      try {
        const user = await addSchedule(data);
        console.log("Jadwal berhasil ditambahkan:", user);

        toggleSuccessModal();

        // Mengganti Tabs setelah 2 detik
        setTimeout(() => {
          navigation.navigate("Edit_jadwal", {
            scheduleData: {
              activity,
              sdescription,
              selectedSCategory: selectedBox,
