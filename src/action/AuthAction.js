...
export const deleteSchedule = async (scheduleId, userSchedules, setUserSchedules) => {
  try {
    const userData = await getData('user');
    ...
    const scheduleRef = FIREBASE.database().ref(`schedule/${userData.uid}/${scheduleId}`);
    ...
    const updatedSchedules = userSchedules.filter(
...
export const editSchedule = async (scheduleId, updatedData) => {
  try {
    // Ambil data pengguna yang sudah login dari fungsi 'getData'
    const userData = await getData("user");

    if (userData) {
      // Perbarui jadwal berdasarkan scheduleId
      const scheduleRef = FIREBASE.database().ref(`schedule/${userData.uid}/${scheduleId}`);
      const snapshot = await scheduleRef.once("value");
      const existingSchedule = snapshot.val();
